...
if theme_mode == "Dark":
    st.markdown("""
        ...
        [data-testid="stMetricValue"] {color: #E03A3E !important;}
        </style>
    """, unsafe_allow_html=True)
else:
    st.markdown("""
        <style>
        [data-testid="stMetricValue"] {color: #0B3D91 !important;}
        </style>
    """, unsafe_allow_html=True)
...
# HEADER
# -------------------------------------------------------------------
st.markdown(f"<h1 style='text-align:center;'>🏀 NBA Player Performance Predictor Complete</h1>", unsafe_allow_html=True)
st.markdown(f"<p style='text-align:center; color:gray;'>Powered by NBA API Data • Updated {datetime.now().strftime('%B %d, %Y')}</p>", unsafe_allow_html=True)
...
# -------------------------------------------------------------------
# ENHANCEMENT: FETCH PLAYER HEADSHOT & TEAM LOGOS
# -------------------------------------------------------------------
...
def get_team_logo(team_name):
    team_info = next((t for t in teams.get_teams() if t["full_name"] == team_name), None)
    if not team_info:
        return None
...
def plot_shot_chart(player_id, season="2023-24", season_type="Regular Season"):
    ...
    sns.kdeplot(x=df_shots["LOC_X"] / 10, y=df_shots["LOC_Y"] / 10,
                fill=True, cmap="Reds", alpha=0.6, ax=ax2)
...
# -------------------------------------------------------------------
# SIDEBAR + INTERACTIVE LAYOUT (UNCHANGED SECTIONS KEPT)
# -------------------------------------------------------------------
...
# Header area with player image & team logo
if selected_player != "Select a player...":
    player_id = get_player_id(selected_player)
    player_img = get_player_image(player_id)
    team_logo = get_team_logo(selected_team)
    c1, c2, c3 = st.columns([1, 2, 1])
    with c1:
        if player_img:
            st.image(player_img, caption=selected_player, width=180)
    with c2:
        st.subheader(f"{selected_player} vs {selected_team}")
    with c3:
        if team_logo:
...
with tabs[6]:
    ...
    - Player headshots and team logos directly from the NBA CDN  

    Built with Streamlit and nba_api using modern visualization frameworks.
    """)
